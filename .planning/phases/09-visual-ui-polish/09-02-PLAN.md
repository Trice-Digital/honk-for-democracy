...
<objective>
Replace the colored rectangle cars with paper cutout top-down car shapes. This is the highest visual impact upgrade — cars are the most visible moving objects.
...
2. Draw drop shadow first using `drawPaperShadow()` — offset rectangle matching car bounds
...
7. Stroke outline with `PALETTE.markerBlack` at 2px — the marker outline look
8. Add this Graphics object to the Container
...
- Add: `public carType: CarType` — exposed so ReactionSystem can weight reactions based on car type
...
**Performance note:**
- Cars are drawn once at construction and on reset — not every frame
- The Graphics object is part of the Container, moves with it — no per-frame redraw needed
