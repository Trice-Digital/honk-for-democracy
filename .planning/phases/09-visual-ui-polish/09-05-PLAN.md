...
- Use fontFamily `'Bangers', cursive` (already loaded for sign creator)
...
**"STOPPED TRAFFIC" score multiplier banner:**
- When traffic lights are RED (all-stop phase), show a paper cutout banner at the top of the gameplay area:
  - Text: "STOPPED TRAFFIC x2" in Bangers font, PALETTE.safetyYellow with marker black stroke
  - Paper cutout rectangle background with scissor-cut edges and drop shadow
  - Slide in from top (tween y from -40 to 10, duration 300ms)
  - Remove when lights go green (slide back up)
- This is purely visual — the actual score multiplier logic should already exist or be a simple `isRedPhase ? 2 : 1` multiplier in the score calculation

**Car behavior contrast (stopped vs moving):**
- Stopped cars (at red light): emoji face turns to look at player sign
  - Rotate the emoji text slightly toward the player's position (simple angle calc)
  - Reactions from stopped cars are more frequent and larger speech bubbles
- Moving cars (at green light): emoji face stays forward-facing, smaller/faster reactions
- This makes the red/green strategic difference VISIBLE, not just mechanical

**Do NOT change the reaction logic** — only the visual rendering in showReactionFeedback. The car behavior contrast is visual emphasis only (reaction frequency is handled by existing ReactionSystem config).
  </action>
  <verify>
`npm run build` succeeds. Reactions display in paper speech bubbles. Score floaters use marker-font styling. Reactions animate with paper flutter.
  ...
  <done>
Reactions show in paper speech bubbles with scissor-cut edges and drop shadows. Score floaters use Bangers font with bounce animation. Paper flutter on float-away.
  </done>
</task>
...
- Labels: use `'Bangers', cursive` fontFamily
